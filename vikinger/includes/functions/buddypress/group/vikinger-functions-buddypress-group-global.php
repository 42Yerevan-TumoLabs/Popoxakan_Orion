...
if (!function_exists('vikinger_group_types_get')) {
  /**
   * Returns group types.
   * 
   * @since 1.9.8.1
   * 
   * @return array $group_types        Group types.
   */
  function vikinger_group_types_get() {
    $group_types = [];
...
if (!function_exists('vikinger_group_get_group_status_data')) {
  ...
  function vikinger_group_get_group_status_data($bp_group) {
    $group_status_data = [
      ...
      'description'         => html_entity_decode($bp_group['description']['raw']),
      'slug'                => $bp_group['slug'],
      'link'                => $bp_group['link'],
...
if (!function_exists('vikinger_group_get_data')) {
  ...
  function vikinger_group_get_data($bp_group, $data_scope = 'group-status') {
    ...
    $group_data['group_types'] = $bp_group['types'];
    ...
    if (!is_null($results)) {
      foreach ($results as $user_group) {
        $user_group_data = [
          ...
          'name'          => stripslashes(html_entity_decode($user_group->name)),
...
if (!function_exists('vikinger_group_update')) {
  ...
  function vikinger_group_update($args) {
    ...
    if ($result) {
      ...
      if (array_key_exists('status', $args)) {
        if ($args['status'] === 'public') {
          vikinger_group_activities_show_sitewide($group_id);
        } else {
          vikinger_group_activities_hide_sitewide($group_id);
        }
      }

      // update group types
      if (array_key_exists('types', $args)) {
        bp_groups_set_group_type($group_id, $args['types']);
      }
