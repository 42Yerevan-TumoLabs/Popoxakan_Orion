	...
	<div class="page" id="register-page">
		...
		<?php if ( 'registration-disabled' == bp_get_current_signup_step() ) : ?>
			...
			do_action( 'bp_before_registration_disabled' ); ?>
				...
				<p><?php _e( 'User registration is currently not allowed.', 'vikinger' ); ?></p>
		...
		<?php if ( 'request-details' == bp_get_current_signup_step() ) : ?>
			...
			<p><?php _e( 'Registering for this site is easy. Just fill in the fields below, and we\'ll get a new account set up for you in no time.', 'vikinger' ); ?></p>
			...
			<div class="register-section" id="basic-details-section">
				...
				<h2><?php _e( 'Account Details', 'vikinger' ); ?></h2>

				<label for="signup_username"><?php _e( 'Username', 'vikinger' ); ?> <?php _e( '(required)', 'vikinger' ); ?></label>
				<?php
				...
				<label for="signup_email"><?php _e( 'Email Address', 'vikinger' ); ?> <?php _e( '(required)', 'vikinger' ); ?></label>
				<?php
				...
				<label for="signup_password"><?php _e( 'Choose a Password', 'vikinger' ); ?> <?php _e( '(required)', 'vikinger' ); ?></label>
				<?php
				...
				<label for="signup_password_confirm"><?php _e( 'Confirm Password', 'vikinger' ); ?> <?php _e( '(required)', 'vikinger' ); ?></label>
				<?php
			...
			<?php if ( bp_is_active( 'xprofile' ) ) : ?>
				...
				<div class="register-section" id="profile-details-section">
					...
					<h2><?php _e( 'Profile Details', 'vikinger' ); ?></h2>
			...
			<?php if ( bp_get_blog_signup_allowed() ) : ?>
				...
				<div class="register-section" id="blog-details-section">
					...
					<h2><?php _e( 'Blog Details', 'vikinger' ); ?></h2>

					<p><label for="signup_with_blog"><input type="checkbox" name="signup_with_blog" id="signup_with_blog" value="1"<?php if ( (int) bp_get_signup_with_blog_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'Yes, I\'d like to create a new site', 'vikinger' ); ?></label></p>

					<div id="blog-details"<?php if ( (int) bp_get_signup_with_blog_value() ) : ?>class="show"<?php endif; ?>>

						<label for="signup_blog_url"><?php _e( 'Blog URL', 'vikinger' ); ?> <?php _e( '(required)', 'vikinger' ); ?></label>
						<?php
						...
						<label for="signup_blog_title"><?php _e( 'Site Title', 'vikinger' ); ?> <?php _e( '(required)', 'vikinger' ); ?></label>
						<?php
						...
						<fieldset class="register-site">
							<legend class="label"><?php _e( 'Privacy: I would like my site to appear in search engines, and in public listings around this network.', 'vikinger' ); ?></legend>
							<?php
							...
							<label for="signup_blog_privacy_public"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_public" value="public"<?php if ( 'public' == bp_get_signup_blog_privacy_value() || !bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'Yes', 'vikinger' ); ?></label>
							<label for="signup_blog_privacy_private"><input type="radio" name="signup_blog_privacy" id="signup_blog_privacy_private" value="private"<?php if ( 'private' == bp_get_signup_blog_privacy_value() ) : ?> checked="checked"<?php endif; ?> /> <?php _e( 'No', 'vikinger' ); ?></label>
						</fieldset>
			...
			<div class="submit">
				<input type="submit" name="signup_submit" id="signup_submit" value="<?php esc_attr_e( 'Complete Sign Up', 'vikinger' ); ?>" />
			</div>
		...
		<?php if ( 'completed-confirmation' == bp_get_current_signup_step() ) : ?>
			...
			<div id="template-notices" role="alert" aria-atomic="true">
				<?php if ( bp_registration_needs_activation() ) : ?>
					<p><?php _e( 'You have successfully created your account! To begin using this site you will need to activate your account via the email we have just sent to your address.', 'vikinger' ); ?></p>
				<?php else : ?>
					<p><?php _e( 'You have successfully created your account! Please log in using the username and password you have just created.', 'vikinger' ); ?></p>
				<?php endif; ?>
			</div>
