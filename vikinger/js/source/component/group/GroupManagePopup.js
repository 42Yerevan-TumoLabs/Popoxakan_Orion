...
const groupTypes = Object.values(vikinger_constants.settings.group_types).filter(groupType => groupType.show_in_create_screen === '1');
...
function GroupManagePopup(props) {
  ...
  const getGroupData = () => {
    ...
    .done((response) => {
      ...
      defaultGroupData.current.types = groupResponseData.group_types;
  ...
  const handleGroupTypeChange = (groupType) => {
    setGroupData(previousGroupData => {
      ...
      if (previousGroupData.types.includes(groupType.name)) {
        newGroupTypes = previousGroupData.types.filter(previousGroupType => previousGroupType !== groupType.name);
      } else {
        newGroupTypes = previousGroupData.types.concat([groupType.name]);
      }
  ...
  const getGroupDataUpdatedFields = () => {
    ...
    if (updatedFields.types instanceof Array && updatedFields.types.length === 0) {
      updatedFields.types = '';
    }
  ...
  const save = () => {
    ...
    if (creatingGroup) {
      ...
      // handle group types
      if (createGroupConfig.types instanceof Array) {
        createGroupConfig.types = createGroupConfig.types.join(',');
      }

      const createGroupPromise = groupRouter.createGroup(createGroupConfig);

      // console.log('GROUP MANAGE POPUP - CREATE GROUP WITH DATA: ', createGroupConfig);
                        ...
                        <div className="form-row">
                          <div className="form-item">
                            <div className="form-input-list">
                              <p className="form-input-list-title">{vikinger_translation.group_type}</p>
                              ...
                              <div className="form-input-list-items">
                              {
                                groupTypes.map((groupType) => {
                                  return (
                                    <FormInputCheckbox
                                      key={groupType.name}
                                      text={groupType.labels.singular_name}
                                      active={groupData.types.includes(groupType.name)}
                                      toggleActive={() => { handleGroupTypeChange(groupType); }}
                                    />
                                  )
                                })
                              }
